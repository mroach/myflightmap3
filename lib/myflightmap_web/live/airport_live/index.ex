defmodule MyflightmapWeb.AirportLive.Index do
  ...
  def load_paginated_airports(socket, query, pagination \\ []) do
    current_page = Keyword.get(pagination, :page, 0)
    pagination_opts = [page_size: 50, page: current_page]

     {pagination, airports} = case query do
      blank when blank in [nil, ""] ->
        {:ok, %{pagination: pagination, records: airports}} =
          T.list_airports(pagination: pagination_opts)
        {pagination, airports}
      term ->
        {:ok, %{pagination: pagination, records: records}} =
          T.search_airports_paginated(term, pagination: pagination_opts)

        airports = Enum.map(records, &(&1.airport))
        {pagination, airports}
      end
  ...
  def handle_event("page-prev", _, socket) do
    ...
    socket = load_paginated_airports(socket, query, pagination_opts)
    {:noreply, socket}
  end
  ...
  def handle_event("search", %{"value" => query}, socket) do
    socket = load_paginated_airports(socket, query)
    {:noreply, socket}
  end
            ...
            <tbody>
              <%= for airport <- @airports do %>
                <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700">
                  <td class="py-4 px-6"><%= airport.iata_code %></td>
                  <td class="py-4 px-6"><%= airport.icao_code %></td>
                  <td class="py-4 px-6"><%= airport.common_name %></td>
                  <td class="py-4 px-6">
                    <%= Unicode.emoji_flag(airport.country) %>
                    <%= airport.city %>
                  </td>
                </tr>
              <% end %>
