...
alias Myflightmap.Transport.{Aircraft, Airline, Airport}
...
|> Enum.map(fn line ->
  ...
  %Aircraft{iata_code: iata, icao_code: icao, description: desc}
end)
|> Enum.each(fn x ->
  Myflightmap.Repo.insert(x, on_conflict: :replace_all, conflict_target: :iata_code)
end)

for path <- Path.wildcard("priv/data/icao/aircraft/*.json") do
  ...
  |> Enum.each(fn data ->
    ...
    from(a in Aircraft, where: a.icao_code == ^icao_code)
